...
function SmallScreenCard({
  ...
  return (
    <UnstyledLink onClick={() => navigate(`/photos/${id}`)}>
      <div
...
export function PhotoIndex({ Photos, loading }) {
  ...
  /* Map over Photos for small screen */
  const smallScreenMarkup = Photos.map((QRCode) => (
    ...
    ({ id, title, product, discountCode, scans, createdAt }, index) => {
      ...
      return (
        ...
        >
          ...
          <IndexTable.Cell>
            <UnstyledLink data-primary-link url={`/photos/${id}`}>
              {truncate(title, 25)}
            </UnstyledLink>
          </IndexTable.Cell>
          <IndexTable.Cell>
